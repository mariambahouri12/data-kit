...
class DataVisualizer:
    """
    Data visualizer for exploration and analysis.
    """
    ...
    def plot_missing_values(
        self, X: pd.DataFrame, title: str = "Missing Values"
    ) -> plt.Figure:
    ...
    def plot_boxplot(
        ...
        fig, ax = plt.subplots(
            figsize=(max(8, len(columns) * 0.8), 6)
        )
    ...
    def plot_pca(
        ...
        if len(numeric_cols) < 2:
            ...
            ax.text(
                0.5,
                0.5,
                "⚠️ Not enough numeric columns for PCA",
                ha="center",
                va="center",
                fontsize=16,
            )
    ...
    def plot_class_balance(
        ...
        if len(counts) >= 2:
            ...
            fig.suptitle(
                f"{title} (Imbalance Ratio: {imbalance_ratio:.2f})",
                fontsize=14,
                y=1.02,
            )
        else:
    ...
    def plot_feature_importance(
        ...
        fig, ax = plt.subplots(
            figsize=(10, max(4, len(sorted_importances) * 0.3))
        )
    ...
    def plot_data_summary(
        ...
        """Visualize a complete summary of the dataset."""
        ...
        ax1.pie(
            dtypes.values,
            labels=[str(t) for t in dtypes.index],
            autopct="%1.1f%%",
        )
        ...
        if len(missing) > 0:
            ...
            ax2.set_title(
                f"Missing Values (n={len(missing)})",
                fontsize=12,
            )
        ...
        ax3.text(
            0.1,
            0.3,
            f"Memory: {X.memory_usage(deep=True).sum() / 1024**2:.2f} MB",
            fontsize=14,
            va="center",
        )
        ...
        if len(numeric_cols) > 0:
            stats = X[numeric_cols].describe().T[
                ["mean", "std", "min", "max"]
            ]

            ax4.table(
                cellText=stats.round(2).values[:5],
        ...
        if len(categorical_cols) > 0:
            ...
            ax5.bar(
                [c[0] for c in top_cats],
                [c[1] for c in top_cats],
                color="#FFE66D",
            )
            ...
            ax5.set_title(
                f"Categorical Columns (n={len(categorical_cols)})",
                fontsize=12,
            )
        ...
        if n_duplicates > 0:
            ...
            ax6.text(
                0.1,
                0.5,
                f"Percentage: {n_duplicates / len(X) * 100:.2f}%",
                fontsize=14,
                va="center",
            )
