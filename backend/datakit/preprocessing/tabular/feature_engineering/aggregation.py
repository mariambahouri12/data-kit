...
class AggregationFeatureCreator(BasePreprocessor):
    """Creates features through grouped aggregation (e.g., column mean per group)."""
    ...
    def _fit(
        ...
        if (
            self.group_column is None
            or self.group_column not in X.columns
        ):
            raise ValueError(
                f"Group column '{self.group_column}' not found"
            )

        agg_columns = self.agg_columns

        if agg_columns is None:
            agg_columns = X.select_dtypes(
                include=[np.number]
            ).columns.tolist()
        ...
        # Reset: prevents accumulation if _fit is called multiple times
        # on the same object.
        self.agg_mapping = {}
        self.agg_names = []
    ...
    def _transform(
        ...
        for col, agg_dict in self.agg_mapping.items():
            for agg, mapping in agg_dict.items():
                X_copy[f"{col}_{agg}"] = (
                    X_copy[self.group_column].map(mapping)
                )
