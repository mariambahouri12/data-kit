...
class FeatureSelector(BasePreprocessor):
    ...
    - variance: removes numerical columns with near-zero variance.
    - correlation: keeps columns most correlated with the target
      (ANOVA F-test for classification, Pearson for regression).
    - importance: keeps the most important columns according to a RandomForest.
    - rfe: recursive feature elimination using a RandomForest.
    """
    ...
    def _fit_correlation(
        ...
        """
        Selects columns most correlated with the target:
        ANOVA F-test (f_classif) for classification,
        Pearson correlation (f_regression) for regression.
        """
        if y is None:
            self.selected_features = X.columns.tolist()
            return
        ...
        sorted_cols = sorted(
            zip(numeric_cols, f_scores),
            key=lambda x: x[1],
            reverse=True
        )

        selected = self._top_k_or_threshold(
            sorted_cols
        )
    ...
    def _fit_rfe(
        ...
        # Required: RFE internally fits a sklearn model.
        X_encoded = encode_categoricals(X)
        ...
        self.selected_features = (
            X_encoded.columns[mask].tolist()
        )

    def _top_k_or_threshold(
        self,
        ...
        if self.k:
            return [
                col
                for col, _ in sorted_scores[:self.k]
            ]
